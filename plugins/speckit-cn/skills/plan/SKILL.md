...
2. **加载上下文**：读取 FEATURE_SPEC 和 !`node -e "console.log(require('fs').existsSync('.specify/memory/constitution.md') ? require('path').resolve('.specify/memory/constitution.md') : '${WAVE_SKILL_DIR}/../../memory/constitution.md')"`。从 `${WAVE_SKILL_DIR}/../../templates/plan-template.md` 加载 IMPL_PLAN 模板。
