...
function readVersion() {
  ...
  const manifests = [
    ...
    path.join(root, '.wave-plugin/plugin.json')
  ];
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Packaged plugins omit package.json; try the next manifest.
    }
  }
...
function brandMarkup() {
  const version = escapeHtmlText(VERSION);
  const text = 'Brainstorm v' + version;
